    ...
    for line, label in zip(lines, labels):
        ...
        try:
            if reco in reco2segs:
                reco2segs[reco] = "{} {},{},{}".format(
                    reco2segs[reco], start, end, label
                )  # reco2segs[reco] + " " + start + "," + end + "," + label
            else:
                reco2segs[reco] = "{} {},{},{}".format(
                    reco, start, end, label
                )  # reco + " " + start + "," + end + "," + label
        except KeyError:
    ...
    with open(rttm_file, "w") as rttm_writer:
        for reco_line in merged_segs:
            ...
            for i in range(1, len(segs)):
                ...
                print(
                    "LANGUAGE {0} {1} {2:7.3f} {3:7.3f} <NA> <NA> L{4} <NA> <NA>".format(
                        reco,
                        rttm_channel,
                        float(start),
...
def merge_adjacent_labels(starts, ends, labels):
    ...
    adjacent_or_overlap = np.logical_or(
        np.isclose(ends[:-1], starts[1:]), ends[:-1] > starts[1:]
    )
    to_split = np.nonzero(
        np.logical_or(~adjacent_or_overlap, labels[1:] != labels[:-1])
    )[0]
    ...
    ends[overlaping] = starts[overlaping + 1] = (
        ends[overlaping] + starts[overlaping + 1]
    ) / 2.0
...
def get_args():
    ...
    parser.add_argument(
        "embeddings_segments_list"
    )  # emb_segments.list or llk_segments.list
    parser.add_argument(
        "rttm_path"
    )  # exps/exp1/ecapa_tdnn_voxlingua_speechbrain_language_embeddings/rttm_outputs
