...
def pyannote_vad(input_wav, output_dir):
    ...
    # Get auth token from environment variable or prompt user
    auth_token = os.environ.get("PYANNOTE_AUTH_TOKEN")
    if not auth_token:
        print("WARNING: PYANNOTE_AUTH_TOKEN environment variable not set.")
        print("Please set it with: export PYANNOTE_AUTH_TOKEN='your_token'")
        auth_token = None  # Will prompt for login if None
